...
public class L1WeaponSkill {
	...
	private int _effectTarget; // 효과의 대상 0:상대 1:자신
	...
	public static double getWeaponSkillDamage(L1PcInstance pc, L1Character cha,
		...
		if (skillId != 0) {
			...
			if (skill != null && skill.getTarget().equals("buff")) {
				if (! isFreeze(cha)) { // 동결 상태 or카운터 매직중
					cha.setSkillEffect(skillId, weaponSkill
							. getSkillTime() * 1000);
					//actionId = ActionCodes.ACTION_SkillBuff; // 보조 마법 모션
				}
			}
		}
		...
		/*if (damage != 0) {
			actionId = ActionCodes.ACTION_SkillAttack; // 공격 마법 모션
		}*/

		int area = weaponSkill.getArea();
		if (area > 0 || area == -1) { // 범위의 경우
			for (L1Object object : L1World.getInstance()
				...
				if (object.getId() == cha.getId()) { // 공격 대상은 L1Attack로 처리하기 위해(때문에) 제외
					continue;
				}

				// 공격 대상이 MOB의 경우는, 범위내의 MOB에게만 맞는다
				// 공격 대상이 PC, Summon, Pet의 경우는, 범위내의 PC, Summon, Pet, MOB에 해당된다
		...
		/*if (actionId != 0) {
			...
			pc.broadcastPacket(new S_DoActionGFX(pc.getId(), actionId));
		}*/
	...
	public static void giveFettersEffect(L1Character cha) {
		...
		if ((_random.nextInt(100) + 1) <= 2) {
			...
			if (cha instanceof L1PcInstance) {
				...
				pc.setSkillEffect(STATUS_FREEZE, fettersTime);
	...
	public static double getChainSwordDamage(L1PcInstance pc) {
		double dmg = 0;
		int chance = _random.nextInt(100) + 1;
		if (70 >= chance) {
			if(pc.hasSkillEffect(ChinSword1)){
				pc.killSkillEffectTimer(ChinSword1);
				pc.setSkillEffect(ChinSword2, 16 * 1000);
				pc.sendPackets(new S_ChinSword(2));
			}else if (pc.hasSkillEffect(ChinSword2)){
				pc.killSkillEffectTimer(ChinSword2);
				pc.setSkillEffect(ChinSword3, 16 * 1000);
				pc.sendPackets(new S_ChinSword(3));
			}else if (pc.hasSkillEffect(ChinSword3)){
				pc.killSkillEffectTimer(ChinSword3);
				pc.setSkillEffect(ChinSword3, 16 * 1000);
				pc.sendPackets(new S_ChinSword(3));
			}else {
				pc.setSkillEffect(ChinSword1, 16 * 1000);
				pc.sendPackets(new S_ChinSword(1));
			}
		}
		if (pc.hasSkillEffect(ChinSword1)) {
			dmg += 1;
		}
		if (pc.hasSkillEffect(ChinSword2)) {
			dmg += 3;
		}
		if (pc.hasSkillEffect(ChinSword3)) {
			dmg += 5;
		}
		return dmg;
	}
