...
public class L1PolyMorph {
	...
	private static final int BOW_EQUIP = 256; // 간트렛트 포함한다
	
	private static final int QOO_EQUIP = 512; // 키링크
	...
	private static final int GUARDER_EQUIP = 2048; // 가더	 
	...
	static {
		...
		//armorFlgMap.put(13, GUARDER_EQUIP); // 가더
	}  
...
private static void eventusePolyScroll(L1PcInstance pc, String ak) {
  if (pc == null || pc.isDead()) {
   return;
  }
  L1PolyMorph poly = PolyTable.getInstance().getTemplate(ak);
                L1ItemInstance item = pc.getInventory().getItem(pc.getId());
  if (poly != null || ak.equals("")) {
   if (ak.equals("")) {
    pc.removeSkillEffect(L1SkillId.SHAPE_CHANGE);
    pc.sendPackets(new S_CloseList(pc.getId()));
    
   } else if (pc.getLevel() >= poly.getMinLevel() || pc.isGm() || item == null) {
    doPoly(pc, poly.getPolyId(), 1800, ARCH_MORPH_BY_ITEMMAGIC);
    pc.sendPackets(new S_CloseList(pc.getId()));
   ...
   //}
	...
	public static void doPoly(L1Character cha, int polyId, int timeSecs,
		...
		if (cha instanceof L1PcInstance) {
			...
			if (pc.getMapId() == 5302) { // 낚시터
				pc.sendPackets(new S_ServerMessage(1170)); // 여기에서는 변신할 수 없습니다.
				return;
			}
		/*	if (pc.getTempCharGfx() == 6034
			...
			if (pc.getTempCharGfx() != polyId) { // 같은 변신의 경우는 아이콘 송신 이외가 필요없다
			...
			if (mob.getTempCharGfx() != polyId) { // 같은 변신의 경우는 아이콘 송신 이외가 필요없다
	...
	// 지정한 polyId가 weapontType의 무기를 장비 할 수 있을까?
	...
	// 지정한 polyId가 armorType의 방어용 기구를 장비 할 수 있을까?
