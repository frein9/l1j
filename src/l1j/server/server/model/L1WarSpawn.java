...
public class L1WarSpawn {
	...
	public void SpawnTower(int castleId) {
		...
		L1Npc l1npc = NpcTable.getInstance().getTemplate(npcId); // 가디안 타워
		int[] loc = new int[3];
	...
	private void spawnSubTower() {
		...
		int[] loc = new int[3];
		for (int i = 1; i <= 4; i++) {
			l1npc = NpcTable.getInstance().getTemplate(81189 + i); // 서브 타워
	...
	public void SpawnCrown(int castleId) {
		L1Npc l1npc = NpcTable.getInstance().getTemplate(81125); // 크라운
		int[] loc = new int[3];
		loc = L1CastleLocation.getTowerLoc(castleId);
		SpawnWarObject(l1npc, loc[0], loc[1], (short) (loc[2]));
	}
	...
	public void SpawnFlag(int castleId) {
		L1Npc l1npc = NpcTable.getInstance().getTemplate(81122); // 기
		int[] loc = new int[5];
