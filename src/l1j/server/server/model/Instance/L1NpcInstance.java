...
import l1j.server.server.serverpackets.S_ChatPacket;//잠므 버땅
import l1j.server.server.serverpackets.S_NpcChatPacket;//잠므 버땅
		...
		for (Object knownObject : targetKnownObjects) {
			if (knownObject instanceof L1NpcInstance) {
				...
				if (mobGroupInfo != null) {
					if (getMobGroupId() != 0
							&& getMobGroupId() == npc.getMobGroupId()) { // 같은 그룹
						npc.setLink(targetPlayer);
					}
				}
			}
	...
	public void attackTarget(L1Character target) {
		...
		if (attack.calcHit()) {
			if (target.hasSkillEffect(L1SkillId.MOTALBODY)) {
				L1Magic magic = new L1Magic(target, this);
				boolean isProbability = magic
						.calcProbabilityMagic(L1SkillId.MOTALBODY);
				boolean isShortDistance = attack.isShortDistance();
				if (isProbability && isShortDistance) {
					isMotalbody = true;
				}
	...
	public void searchTargetItem() {
		...
		for (L1ItemInstance item : inventory.getItems()) {
			if (getInventory().checkAddItem(item, item.getCount())
					== L1Inventory.OK) { // 가질 수 있다면 타겟 아이템에 가세한다
				_targetItem = item;
	...
	public boolean noTarget() {
		if (_master != null && _master.getMapId() == getMapId()
				&& getLocation().getTileLineDistance(_master
						.getLocation()) > 2) { // 주인이 같은 맵에 있어 멀어지고 있는 경우는 추적
		...
		/////////////////////////////////////////////////////////////// 기르타스 잠수위해 수정 - 몹스킬패턴 수정 : 시작  
			...
			if (getCurrentHp() >= getMaxHp() / 3) {           // 안타라스 
				...
				if (getCurrentMp() >= getMaxMp() / 2) { //엠피          // 안타라스(리뉴얼)
					if (pc.getLocation().getTileLineDistance(this.getLocation()) >= 3
						&& pc.getLocation().getTileLineDistance(this.getLocation()) < 4
						) {
						appearOnGround(pc);
					}
				}
...
// }
	...
	public void appearOnGround(L1PcInstance pc) {
	    ...
	    ////////////////////////////////////////////////////////////////// 기르타스 잠수위해 추가 - 몹스킬패턴 추가 : 시작  
			...
			|| getHiddenStatus() == HIDDEN_STATUS_DOLGOLLEM
			|| getHiddenStatus() == HIDDEN_STATUS_SINK_ANTA_NEW
			) {										// 안타라스, 얼음성 몹, 돌골렘
			setHiddenStatus(HIDDEN_STATUS_NONE);
			broadcastPacket(new S_DoActionGFX(getId(), 11));
			setStatus(0);
		...
		if (dir >= 0) {
			...
			if (getMovementDistance() > 0) {
				if (this instanceof L1GuardInstance
						...
						|| this instanceof L1RguardInstance) {//카시 경비병
