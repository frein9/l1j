	...
	public boolean noTarget() {
		...
		for (L1Object object : L1World.getInstance().getVisibleObjects(this)) {
			if (object instanceof L1NpcInstance) {
				...
				if (npc.getNpcTemplate().get_npcId() == 70740 // 디카르덴소르쟈
						&& getNpcTemplate().get_npcId() == 71093) { // 조사원
					setParalyzed(true);
					...
					deleteMe();
					return true;
				} else if (npc.getNpcTemplate().get_npcId() == 70811 // 라이라
						&& getNpcTemplate().get_npcId() == 71094) { // 엔디아
					setParalyzed(true);
					L1PcInstance pc = (L1PcInstance) _master;
					...
					deleteMe();
					return true;
				} else if (npc.getNpcTemplate(). get_npcId() == 71061 // 모퉁이 모스
						&& getNpcTemplate(). get_npcId() == 71062) { // 카밋트
				...
				} else if (npc.getNpcTemplate(). get_npcId() == 71074 // 리자드만의 장로
						&& getNpcTemplate(). get_npcId() == 71075) { 
					// 완전히 지쳐 버린 리자드만파이타
					if (getLocation(). getTileLineDistance(_master.getLocation()) < 3) {
					L1PcInstance pc = (L1PcInstance) _master;
						if((pc.getX() >= 32731 && pc.getX() <= 32735) // 리자드만 장로 주변 좌표
				...
				} else if (npc.getNpcTemplate(). get_npcId() == 70964 // 밧슈
						&& getNpcTemplate(). get_npcId() == 70957) { // 로이
					if (getLocation(). getTileLineDistance(_master.getLocation()) < 3){
						L1PcInstance pc = (L1PcInstance) _master;	
