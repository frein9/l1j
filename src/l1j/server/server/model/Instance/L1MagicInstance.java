...
public class L1MagicInstance {
    public void UseMagicAttacke(L1PcInstance player, L1MonsterInstance npc) {
        ...
        if (rangeMonster == 1) {
            ...
            npc.broadcastPacket(new S_AttackPacket(player, targetobjid, ActionCodes.ACTION_SkillAttack));
        } else {
            if (magicMob == 99) {
        ...
        if (useMagic > 80) {
            ...
            if (useMagicA < 25) {
                ...
                npc.broadcastPacket(new S_AttackPacket(player, targetobjid, ActionCodes.ACTION_SkillBuff));
                Random random2 = new Random();
                ...
                if (mortalBlow != 0) {
                    ...
                    // mortalBlow));
                    npc.broadcastPacket(new S_SkillSound(player.getId(), mortalBlow));
                    Random random2 = new Random();
                    ...
                    npc.broadcastPacket(new S_AttackPacket(player, targetobjid, ActionCodes.ACTION_AltAttack));
                    Random random2 = new Random();
                    randomMobDmg = (random2.nextInt(mobDmg) + 1) * 2;
                    // CalcHpPlayer (player, npc, randomMobDmg);
                    player.receiveDamage(npc, randomMobDmg, 0);
            ...
            npc.broadcastPacket(new S_AttackPacket(player, targetobjid, ActionCodes.ACTION_Attack));
            Random random = new Random();
