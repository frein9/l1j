...
public class MpRegeneration extends TimerTask {
	...
	public void regenMp() {
		...
		if (_pc.hasSkillEffect(L1SkillId.STATUS_BLUE_POTION) == true) { // 블루 일부 사용중
			if (wis < 11) { // WIS11 미만에서도 MPR+1
				wis = 11;
			}
			baseMpr += wis - 10;
		}
		if (_pc.hasSkillEffect(L1SkillId.MEDITATION) == true) { // 묵상중
			baseMpr += 5;
		}
		if (_pc.hasSkillEffect(L1SkillId.CONSENTRATION)) { // 집중중
			baseMpr += 2;
		}
		...
		if (_pc.getOriginalMpr() > 0) { // 오리지날 WIS MPR 보정
