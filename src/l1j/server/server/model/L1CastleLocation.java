...
public class L1CastleLocation {
	...
	private static final int ADEN_SUB_TOWER1_X = 34057; // 파랑

	private static final int ADEN_SUB_TOWER1_Y = 33291;

	private static final int ADEN_SUB_TOWER2_X = 34123; // 빨강

	private static final int ADEN_SUB_TOWER2_Y = 33291;

	private static final int ADEN_SUB_TOWER3_X = 34057; // 초록

	private static final int ADEN_SUB_TOWER3_Y = 33230;

	private static final int ADEN_SUB_TOWER4_X = 34123; // 흰색
	...
	/**
	 * 전쟁 에리어(기내)의 좌표로부터 castle_id를 돌려준다
	 */
	...
	public static int[] getWarArea(int castleId) {
		int[] loc = new int[5];
		if (castleId == KENT_CASTLE_ID) { // 켄트성
			loc[0] = KENT_X1;
			loc[1] = KENT_X2;
			loc[2] = KENT_Y1;
			loc[3] = KENT_Y2;
			loc[4] = KENT_MAP;
		} else if (castleId == OT_CASTLE_ID) { // 오크의 숲
			loc[0] = OT_X1;
			loc[1] = OT_X2;
			loc[2] = OT_Y1;
			loc[3] = OT_Y2;
			loc[4] = OT_MAP;
		} else if (castleId == WW_CASTLE_ID) { // 윈다웃드성
			loc[0] = WW_X1;
			loc[1] = WW_X2;
			loc[2] = WW_Y1;
			loc[3] = WW_Y2;
			loc[4] = WW_MAP;
		} else if (castleId == GIRAN_CASTLE_ID) { // 기란성
			loc[0] = GIRAN_X1;
			loc[1] = GIRAN_X2;
			loc[2] = GIRAN_Y1;
			loc[3] = GIRAN_Y2;
			loc[4] = GIRAN_MAP;
		} else if (castleId == HEINE_CASTLE_ID) { // Heine성
			loc[0] = HEINE_X1;
			loc[1] = HEINE_X2;
			loc[2] = HEINE_Y1;
			loc[3] = HEINE_Y2;
			loc[4] = HEINE_MAP;
		} else if (castleId == DOWA_CASTLE_ID) { // 드워후성
			loc[0] = DOWA_X1;
			loc[1] = DOWA_X2;
			loc[2] = DOWA_Y1;
			loc[3] = DOWA_Y2;
			loc[4] = DOWA_MAP;
		} else if (castleId == ADEN_CASTLE_ID) { // 에덴성
			loc[0] = ADEN_X1;
			loc[1] = ADEN_X2;
			loc[2] = ADEN_Y1;
			loc[3] = ADEN_Y2;
			loc[4] = ADEN_MAP;
		} else if (castleId == DIAD_CASTLE_ID) { // 디아드 요새
			loc[0] = DIAD_X1;
			loc[1] = DIAD_X2;
			loc[2] = DIAD_Y1;
			loc[3] = DIAD_Y2;
	...
	public static int[] getCastleLoc(int castle_id) { // castle_id로부터 키우치의 좌표를 돌려준다
		int[] loc = new int[3];
		if (castle_id == KENT_CASTLE_ID) { // 켄트성
			loc[0] = 32731;
			loc[1] = 32810;
			loc[2] = 15;
		} else if (castle_id == OT_CASTLE_ID) { // 오크의 숲
			loc[0] = 32800;
			loc[1] = 32277;
			loc[2] = 4;
		} else if (castle_id == WW_CASTLE_ID) { // 윈다웃드성
			loc[0] = 32730;
			loc[1] = 32814;
			loc[2] = 29;
		} else if (castle_id == GIRAN_CASTLE_ID) { // 기란성
			loc[0] = 32724;
			loc[1] = 32827;
			loc[2] = 52;
		} else if (castle_id == HEINE_CASTLE_ID) { // Heine성
			loc[0] = 32568;
			loc[1] = 32855;
			loc[2] = 64;
		} else if (castle_id == DOWA_CASTLE_ID) { // 드워후성
			loc[0] = 32853;
			loc[1] = 32810;
			loc[2] = 66;
		} else if (castle_id == ADEN_CASTLE_ID) { // 에덴성
			loc[0] = 32892;
			loc[1] = 32572;
			loc[2] = 300;
		} else if (castle_id == DIAD_CASTLE_ID) { // 디아드 요새
			loc[0] = 32733;
			loc[1] = 32985;
			loc[2] = 330;
		}
	...
	public static int[] getGetBackLoc(int castle_id) {
		...
		} else { // 존재하지 않는 castle_id가 지정되었을 경우는 SKT
			loc = L1TownLocation
