  ...
  public void run() {
   ...
   for (;;) {
    ...
    L1World.getInstance().broadcastPacketToAll(
      new S_SystemMessage("[******] 월드맵상의 아이템이 30초 후에 삭제됩니다.")); 
    try {
     Thread.sleep(30000);
    } catch (Exception exception) {
     _log.warning("L1DeleteItemOnGround error: " + exception);
    ...
    //#################### 여기 부터 추가 #################
       현재시간();
       L1World.getInstance().broadcastPacketToAll(
         new S_SystemMessage("[******] 월드맵이 깨끗하게 청소되었습니다")); 
       // %2
      try {    
      ...
      // 실제 접속 중인 유저에게 시간 뿌려줘 벌장...
           L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("현재 시간은 " +ampmm+" " + hour +"시 " + minute + "분 " + sec+ "초 " ));
 }
