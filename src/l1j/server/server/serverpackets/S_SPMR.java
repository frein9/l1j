...
public class S_SPMR extends ServerBasePacket {
	...
	private void buildPacket(L1PcInstance pc) {
		...
		if (pc.hasSkillEffect(L1SkillId.STATUS_WISDOM_POTION)) {
			writeC(pc.getSp() - pc.getTrueSp() - 2); // 장비 증가한 SP
		} else {
			writeC(pc.getSp() - pc.getTrueSp()); // 장비 증가한 SP
		}
		int mr = pc.getTrueMr() - pc.getBaseMr();
		  if(mr > 0){
		    writeC(mr); // 장비나 마법으로 증가한 MR
		   } else {
		    writeC(0);
		}
		//writeC(pc.getTrueMr() - pc.getBaseMr()); // 장비나 마법으로 증가한 MR
	}
