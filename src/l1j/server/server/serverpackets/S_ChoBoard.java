...
public class S_ChoBoard extends ServerBasePacket {

	private static final String S_BOARD = "[S] S_Board";

	private static Logger _log = Logger.getLogger(S_ChoBoard.class.getName());

	private byte[] _byte = null;

	public S_ChoBoard(L1NpcInstance board) {
		buildPacket(board, 0);
	}

	public S_ChoBoard(L1NpcInstance board, int number) {
	...
	private void buildPacket(L1NpcInstance board, int number) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM board2 order by id desc");
