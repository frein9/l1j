...
public class S_GameList extends ServerBasePacket {
	...
	private static Logger _log = Logger.getLogger(S_GameList.class.getName());
...
*/
	...
	private void buildPacket1(L1PcInstance pc, int i) {
		...
		for(int k = 0; k < member.size() ; k++){
			if(pc.getId() == member.get(k).getId()){
				member.add(i+1, member.get(k));
				member.remove(k+1);
			}
		}
