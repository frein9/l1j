...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public L1PcInstance loadCharacter(String charName) {
		...
		try {
            ...
            pc.setLastActive(rs.getTimestamp("lastactive")); // 아인하사드의 축복
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO characters SET account_name=?,objid=?,char_name=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,Pay=?,HellTime=?,Banned=?,Karma=?,LastPk=?,DeleteTime=?,OriginalStr=?,OriginalCon=?,OriginalDex=?,OriginalCha=?,OriginalInt=?,OriginalWis=?,sealingPW=?");
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE characters SET level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,LastPk=?,DeleteTime=?,OriginalStr=?,OriginalCon=?,OriginalDex=?,OriginalCha=?,OriginalInt=?,OriginalWis=?,sealingPW=?,lastactive=?,ain_zone=?,ain_point=?,St_Exp=?,PC_Kill=?,PC_Death=? WHERE objid=?");
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
			pstm.setInt(++i, pc.getExp());
			pstm.setInt(++i, pc.getBaseMaxHp());
			...
			pstm.setInt(++i, pc.getOriginalWis());
			pstm.setString(++i, pc.getSealingPW());
			pstm.setTimestamp(++i, pc.getLastActive());  // 아인하사드의 축복
