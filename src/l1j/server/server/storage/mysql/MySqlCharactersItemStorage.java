...
public class MySqlCharactersItemStorage extends CharactersItemStorage {
	...
	public ArrayList<L1ItemInstance> loadItems(int objId) throws Exception {
		...
		try {
			...
			while (rs.next()) {
			    ...
			    item.setUpacse(rs.getInt("upacse"));
			    item.setLockitem(rs.getInt("lockitem"));
	...
	public void storeItem(int objId, L1ItemInstance item) throws Exception {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_items SET id = ?, item_id = ?, char_id = ?, item_name = ?, count = ?, is_equipped = 0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, upacse = ?, lockitem = ? ");
			...
			pstm.setInt(13, item.getUpacse());
			pstm.setInt(14, item.getLockitem());
	...
	@Override
	 public void updateItemAttrEnchantLevel(L1ItemInstance item) throws Exception {
	  executeUpdate(item.getId(), 
	    "UPDATE character_items SET attr_enchantlvl = ? WHERE id = ?", item
	      .getAttrEnchantLevel());
	  item.getLastStatus().updateAttrEnchantLevel();
	 }
	
	 @Override
	 public void updateItemUpacse(L1ItemInstance item) throws Exception {
	  executeUpdate(item.getId(),
	    "UPDATE character_items SET upacse = ? WHERE id = ?", item
	      .getUpacse());
	  item.getLastStatus().updateUpacse();
	 }	 
	 
	 @Override
	 public void updateItemLockitem(L1ItemInstance item) throws Exception {
	    executeUpdate(item.getId(), "UPDATE character_items SET lockitem = ? WHERE id = ?", 
	 item.getLockitem());
	 }
