	...
	private void loadTable() {
		...
		try {
			...
			while (rs.next()) {
				...
				ub.setEnterDragonKnight(rs.getBoolean("enter_dragonknight")); // 용기사 추가
				ub.setEnterBlackWizard(rs.getBoolean("enter_blackwizard"));   // 환술사 추가
		...
		_log.config("UB리스트 " + _ub.size() + "건로드");
	}
