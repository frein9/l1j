...
import java.sql.PreparedStatement; //## A83 �˻� ��ɾ� �߰� ���� ����Ʈ 
...
import java.sql.ResultSet;
import java.sql.Connection; // ##### �����߰� ���� ����Ʈ #######
import l1j.server.Base64; //## A112 ��ȣ���� ��ɾ� �߰����� ����Ʈ 
import l1j.server.Config; //## A66 �ҽ� ���� : (��) ��� �ڵ� �ܺ�ȭ ���� ����Ʈ 
import l1j.server.L1DatabaseFactory; //## A83 �˻� ��ɾ� �߰� ���� ����Ʈ 	
...
import l1j.server.server.model.L1Clan;  // Ŭ�������� �ľ��� ���� ����Ʈ 
...
public class GMCommands {
	...
	private boolean executeDatabaseCommand(L1PcInstance pc, String name,
		...
		try {
			...
			if (pc.getAccessLevel() < command.getLevel()) {
				pc.sendPackets(new S_ServerMessage(74, "Ŀ���" + name)); // \f1%0�� ����� �� �����ϴ�.
				return true;
			}
			...
			_log.info(pc.getName() + "��." + name + " " + arg + "Ŀ��带 ����߽��ϴ�. ");
			return true;
		} catch (Exception e) {
   ...
   gm.sendPackets(new S_SystemMessage(".�����Ϸε� ��� �Է��� �ּ���."));
	...
	public void handleCommands(L1PcInstance gm, String cmdLine) {
		...
		try {
			...
			if (gm.getAccessLevel() < 100) {
				gm.sendPackets(new S_ServerMessage(74, "Ŀ���" + cmd)); // \f1%0�� ����� �� �����ϴ�.
				return;
			}
			...
			if(gm.getInventory().checkEquipped(300000)){   // ����� ���� ���������� ��� ��ɾ� ��밡��
			...
			if (cmd.equalsIgnoreCase("����")) {
			    ...
			    allrecall(gm);
			}else if(cmd.equalsIgnoreCase("ħ������")){
			    ħ������(gm);
    	  ...
    	  /*} else if (cmd.equalsIgnoreCase("�����ۼ�Ʈ")) {
				...
				accountadd(gm, param);
            } else if (cmd.equalsIgnoreCase("��ȣ����")) { //## A112 ��ȣ���� �߰� 
				changePassword(gm, param); 
            } else if (cmd.equalsIgnoreCase("�극�̽�")) {
				PetRace(gm);
			} else  if (cmd.equalsIgnoreCase("�������")) { //��ɾ� �κ� ������ ��ġ�� �߰�.
			    BugRace();
			} else if (cmd.equalsIgnoreCase("�𽺺�")){ //��ڸ�ɾ�𽺺� 
                disbi(gm);
			} else if ( cmd.equalsIgnoreCase("����")) {
				question(gm, param);
			} else if ( cmd.equalsIgnoreCase("���")) {
			...
			//_log.info(gm.getName() + "��." + cmdLine + "�ڸ��带 ����߽��ϴ�.");
	...
	private void showHelp(L1PcInstance pc) {
		...
		pc.sendPackets(new S_SystemMessage(".�ǹ� .�극�̽� .��ȣ���� .�������Ʈ .��  .skick"));
		pc.sendPackets(new S_SystemMessage(".���� .��� .���ͼ�ȯ .�˻� .�����Ϸε� .ħ������"));
	...
	private void StartWar(L1PcInstance gm, String param)
	 ...
	 private void serchdroplist(L1PcInstance gm, String param) {
		  ...
		  //   _log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		   ...
		   gm.sendPackets(new S_SystemMessage("ex) .�������Ʈ ������(����Ƽ�׷���Ʈ) -- > �˻� O"));
		   gm.sendPackets(new S_SystemMessage("ex) .�������Ʈ �� -- > �˻� X"));
		  }
		 }
	...
	private void ħ������(L1PcInstance gm){
	    try {
	     ...
	     gm.sendPackets(new S_SystemMessage("������ǥ�����Ϸ�"));
	     MonsterInvasion.getInstance().InvasionStartON();
	     gm.sendPackets(new S_SystemMessage("ħ������Start"));
	     } catch (Exception e) {
	      gm.sendPackets(new S_SystemMessage("ħ�����ۿ���"));
	     }
	 }

	private void disbi(L1PcInstance pc) {                                               // �Լ� ���� �κ� �Դϴ�.
	         
	   for (L1Object obj : L1World.getInstance().getVisibleObjects(pc, 20)) //���� 20�� �����Դϴ�..ȭ����ü�� �� �ѷ���..
	      {                                                                                         // 20 ���� ���� ������Ʈ�� ã�Ƽ�
	         if (obj instanceof L1MonsterInstance)
	       {                                                                                       // ���� ������Ʈ�� ������ ���� ���Ͷ��..
	          L1NpcInstance npc = (L1NpcInstance) obj;

	          npc.receiveDamage(pc, 200000);                                       // ȭ����� ��� ������ 20�� �������� ������ �Ѹ���.

	          if (npc.getCurrentHp() <= 0)
	        {
	           pc.sendPackets(new S_SkillSound(obj.getId() , 1815));      // �� ���� �׷����� ��� ������ �۷�...
		...
		try {
			...
			if (adena != null) {
				pc.sendPackets(new S_SystemMessage((new StringBuilder())
						.append(count).append("�Ƶ����� �����߽��ϴ�.").toString()));
			}
		} catch (Exception e) {
	...
	private void moveToChar(L1PcInstance gm, String pcName) {
		try {
			...
			if (target != null) {
				...
				gm.sendPackets(new S_SystemMessage((new StringBuilder())
						.append(pcName).append("���� �����ϴ�.").toString()));
			}
		...
		String s = null;
		  try {
		   ...
		   if(clan !=null){
		    ...
		    try {
		    int i;
		     for (i = 0; i < clan_member_name.length; i++) { // ���Ϳ� ����
		     ...
		     gm.sendPackets(new S_SystemMessage(".���ͼ�ȯ Ŀ��� ����")); 
		     return;
		    }
		   }else{
	...
	private void visible(L1PcInstance gm) {
		try {
			...
			L1World.getInstance().broadcastPacketToAll(new S_Invis(gm.getId(), 0)); // �߰�
			gm.sendPackets(new S_SystemMessage("������¸� �����߽��ϴ�."));
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage(".������ Ŀ��� ����"));
		}
	}
	...
	private void invisible(L1PcInstance gm) {
		try {
			...
			L1World.getInstance().broadcastPacketToAll(new S_Invis(gm.getId(), 1)); // �߰�
	...
	private void polymorph(L1PcInstance gm, String param) {
		try {
			...
			if (pc == null) {
				gm.sendPackets(new S_ServerMessage(73, name)); // \f1%0�� ������ �ϰ� ���� �ʽ��ϴ�.
			} else {
				try {
	...
	private void chatng(L1PcInstance gm, String param) {
		try {
			...
			if (pc != null) {
				...
				pc.sendPackets(new S_ServerMessage(286, String.valueOf(time))); // \f3���ӿ� �������� �ʴ� �ൿ�̱� (����)������, ����%0�а� ä���� �����մϴ�.
				gm.sendPackets(new S_ServerMessage(287, name)); // %0�� ä���� �����߽��ϴ�.
				L1World.getInstance().broadcastPacketToAll(
						new S_SystemMessage(""+ name + "���� ���� ä�ñ��� ���Դϴ�."));
			}
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage(
					".ä�� ĳ���͸� �ð�(��)�̶�� �Է��� �ּ���."));
			...
			if (temp != null) {
				if (temp.isStackable()) {
					...
					if (gm.getInventory().checkAddItem(item, count) == L1Inventory.OK) {
						...
						gm.sendPackets(new S_ServerMessage(403, // %0�� �տ� �־����ϴ�.
	...
	private void present(L1PcInstance gm, String param) {
		try {
			...
			gm.sendPackets(new S_SystemMessage(temp.getNameId() + "��" + count
					+ "�� ���� �߽��ϴ�.", true));
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage(
					".���� [��ī��Ʈ��] [������ ID] [��æƮ��] [�����ۼ�]�� �Է� ���ּ���.(��ī��Ʈ���� *���� �ϸ� ��ü ����)"));
		}
	}
	...
	private void lvPresent(L1PcInstance gm, String param) {
		try {
			...
			gm.sendPackets(new S_SystemMessage(temp.getName() + "��" + count
					+ "�� ���� �߽��ϴ�.(Lv" + minlvl + "~" + maxlvl + ")"));
		} catch (Exception e) {
			gm
					.sendPackets(new S_SystemMessage(
							".�������� minlvl maxlvl ������ID ��æƮ�� �����ۼ��� �Է��� �ּ���."));
		}
	}
		...
		try {
			...
			for (L1PcInstance pc : L1World.getInstance(). getVisiblePlayer(gm)) {
				if (pc.getCurrentHp() == 0 && pc.isDead()) {
					...
					// �ູ�� ��Ȱ ��ũ�Ѱ� ���� ȿ��
					pc.setTempID(objid);
					pc.sendPackets(new S_Message_YN(322, "")); // �� ��Ȱ�ϰ� �ͽ��ϱ�? (Y/N)
				} else {
					pc.sendPackets(new S_SystemMessage(
							"��ڰ� �޷� �־����ϴ�."));
			...
			gm.sendPackets(new S_SystemMessage(".��Ȱ Ŀ��� ����"));
		}
	}
	...
	private void kick(L1PcInstance gm, String param) {
		try {
			...
			if (target != null) {
				gm.sendPackets(new S_SystemMessage((new StringBuilder())
						.append(target.getName()).append("���� �߹� �߽��ϴ�.")
						.toString()));
	...
	private void skick(L1PcInstance gm, String pcName) {
		try {
			...
			if (target != null) {
				...
				targetClient.kick();
				_log.warning("GM�� �߹��ɿ� ����(" + targetClient.getAccountName()
						+ ":" + targetClient.getHostname() + ")���� ������ ���� ���� �߽��ϴ�.");
			} else {
				gm.sendPackets(new S_SystemMessage(
						"�׷��� �̸��� ĳ���ʹ� ���峻���� �������� �ʽ��ϴ�."));
			}
	...
	/*private void allBuff(L1PcInstance gm, String args) {
		...
		try {
			...
			gm.sendPackets(new S_SystemMessage(".�ù��� ĳ���͸����� �Է��� �ּ���."));
		}
	}
*/
        		...
        		for (int j = 0; j < enchant.size(); ++j) {
        			if (enchant.get(j).getEnchantLevel() >= enlvl)
        				gm.sendPackets(new S_SystemMessage(pc.getName() + " : "
        						+enchant.get(j).getEnchantLevel()+enchant.get(j).getName()+" ����."));
        		}
        	}
        } catch (Exception e) {
		...
		try {
			...
			for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
				...
				if (adena.getCount() >= money)
					gm.sendPackets(new S_SystemMessage(pc.getName () + " : " + adena.getCount() + " �Ƶ��� ����."));
			}
...
// ########## ���� ���� �߰�           #########
	...
	private void hpBar(L1PcInstance gm, String param) {
		if (param.equalsIgnoreCase("on")) {
			...
			gm.sendPackets(new S_SystemMessage(".�ǹ� on �Ǵ� off �̶�� �Է��� �ּ���."));
		}
	}
	...
	private void showTraps(L1PcInstance gm, String param) {
		if (param.equalsIgnoreCase("on")) {
			...
			gm.sendPackets(new S_SystemMessage(".showtrap on|off ��� �Է��� �ּ���."));
		}
	}
	...
	private void redo(L1PcInstance gm, String param) {
		try {
			if (_lastCmd.isEmpty()) {
				gm.sendPackets(new S_SystemMessage("����ϰ� �ִ� Ŀ��尡 �����ϴ�"));
				return;
			}
			if (param.isEmpty()) {
				gm.sendPackets(new S_SystemMessage("Ŀ��� " + _lastCmd
						+ " (��)�� ������մϴ�"));
				...
				gm.sendPackets(new S_SystemMessage("Ŀ��� " + cmd + " �� �����մϴ�."));
				handleCommands(gm, cmd);
			}
		} catch (Exception e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
			gm.sendPackets(new S_SystemMessage(".r Ŀ��� ����"));
		}
	}
	...
	private void favorite(L1PcInstance gm, String param) {
		try {
			if (param.startsWith("set")) {
				...
				st.nextToken();
				if (!st.hasMoreTokens()) {
					gm.sendPackets(new S_SystemMessage("Ŀ��尡 �ϴ��Դϴ�."));
					return;
				}
				...
				gm.sendPackets(new S_SystemMessage("������ ��� Ŀ���: " + _faviCom));
			...
			gm.sendPackets(new S_SystemMessage(".f set Ŀ���� "
					+ "| .f show | .f [�μ�] ��� �Է��� �ּ���."));
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		}
	}
	 ...
	 private void question(L1PcInstance gm, String pcName) {
		 ...
		 private void result(L1PcInstance gm, String pcName) {
		  try {
		   ...
		   L1World.getInstance().broadcastPacketToAll
		   (new S_SystemMessage("����: "+Config.Quest_Yes + "�� �ݴ�:" + Config.Quest_No +"��  " + b));
